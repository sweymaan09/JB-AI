...
const Message: React.FC<MessageProps> = ({ message }) => {
    ...
    return (
        <div className={`flex items-start gap-4 my-4 ${isAi ? '' : 'flex-row-reverse'}`}>
            <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${isAi ? 'bg-primary text-white' : 'bg-gray-300 text-gray-700'}`}>
            ...
            <div className={`p-4 rounded-lg max-w-lg ${isAi ? 'bg-white shadow-md' : 'bg-primary text-white'}`}>
                ...
                {message.audioBase64 && <AudioPlayer audioBase64={message.audioBase64} />}
            </div>
        </div>
    );
};
